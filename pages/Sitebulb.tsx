...
import { Network, Link as LinkIcon, AlertTriangle, Layers, RefreshCw, Code, Type, Globe, Shield, Hash, FileWarning, ArrowRight, Download } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';

type TabKey = 'overview' | 'redirects' | 'http' | 'www' | 'case' | 'encoding' | 'php' | 'patterns' | 'redirect_pages';
...
export const Sitebulb: React.FC = () => {
    ...
    if (!crawlId) return (
        <div className="flex flex-col items-center justify-center mt-20">
            <div className="p-8 text-center text-slate-400 max-w-md bg-slate-900 border border-slate-800 rounded-xl shadow-lg">
                ...
                <p className="text-sm mb-6 text-slate-500">Select an active project from the dashboard to view the redirect audit.</p>
    ...
    if (loading) return <div className="p-8 flex items-center justify-center gap-3 text-slate-500"><div className="w-4 h-4 border-2 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin" /> Analyzing redirect sources...</div>;
    ...
    const tabs: { key: TabKey; label: string; icon: any; count?: number }[] = [
        ...
        { key: 'encoding', label: 'Bad Encoding (+)', icon: Hash, count: s.encoding_issues },
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                        {items.map((item: any, i: number) => (
                            <tr key={i} className="hover:bg-slate-800/30">
                                <td className="px-3 py-2 text-slate-300 font-mono text-[11px] truncate max-w-xs" title={item.source_url}>
            ...
            <div>
                ...
                <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-bold text-white">Redirect Source Detector</h2>
                    </div>
                    <a href={`/api/export.php?crawl_id=${crawlId}&type=issues`} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-lg text-xs font-medium transition-colors">
                        <Download className="w-3.5 h-3.5" /> Export Issues CSV
                    </a>
                </div>
                <p className="text-sm text-slate-400 mt-1">Finds every internal link causing "Page with redirect" in Google Search Console. Shows exact source page, bad link, and fix.</p>
            </div>
            ...
            {auditData?.summary && (
                <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
                        ...
                        <div className="text-[10px] text-slate-500 mt-0.5">Total Issues</div>
                    </div>
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
                        <div className="text-xl font-black text-amber-400">{s.redirect_links}</div>
                        <div className="text-[10px] text-slate-500 mt-0.5">→ Redirect Links</div>
                    </div>
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
                        <div className="text-xl font-black text-orange-400">{(s.http_links || 0) + (s.www_links || 0)}</div>
                        <div className="text-[10px] text-slate-500 mt-0.5">Protocol Issues</div>
                    </div>
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
                        <div className="text-xl font-black text-purple-400">{s.case_issues}</div>
                        <div className="text-[10px] text-slate-500 mt-0.5">Case Mismatches</div>
                    </div>
                </div>
            )}
                                ...
                                <table className="w-full text-left text-xs">
                                    <thead className="bg-slate-950 text-slate-500 sticky top-0 z-10">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2.5 text-red-400">Bad Link (Redirects)</th>
                                            <th className="px-3 py-2.5 text-green-400">Should Link To</th>
